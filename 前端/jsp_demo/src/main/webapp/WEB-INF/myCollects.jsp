...
<div class="dakuai">
    ...
    <div class="qxsc">
        ...
        <div class="quanxuananniu">
            <input type="button" value="取消收藏" id="delAllBtn" class="qx">
...
<script>
    ...
    function getOrders(no) {
        $.getJSON("myCollects.s",{"pageNo":no},function (res) {
            if(res.code==200){
                //为总页面赋值
                ...
                for(var i=0;i<list.length;i++){
                    ...
                    tmp+='<a href="JavaScript:del('+obj.product_code+');"><br>取消收藏</a>';
                    tmp+='</div></div>';
                }
                $("#myCollectsProduct").html(tmp);

                //单击全选加入购物车事件
                $("#addAllBtn").click(function () {
                    var valueStr="";
                    $("#myCollectsProduct input:checked").each(function () {
                        valueStr+=$(this).val()+"-";
                    });
    ...
    function del(id) {
        //调用后台删除方法
        $.getJSON("myCollectDel.s",{"id":id},function (res) {
            if(res.code==200){
                //弹框显示
                $("#myModalLabel").html("取消收藏");
                $("#myModal_del div.modal-body").html("取消收藏成功！");
    ...
    //批量删除
    function delSelect(obj3) {
        //调用后台删除方法
        $.getJSON("myCollectDelAll.s",{"obj3":obj3},function (res) {
            if(res.code==200){
                //弹框显示
                $("#myModalLabel").html("取消收藏");
                $("#myModal_del div.modal-body").html("取消收藏成功！");
